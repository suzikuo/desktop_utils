...
class Main:
    ...
    def init_label(self):
        ...
        # 选择文件
        select_local_file_button = tk.Button(
            self.root, text="选择文件", command=self.select_local_file
        )
        select_local_file_button.grid(row=0, column=2, padx=5, pady=5, sticky="we")
        # 选择本地文件夹按钮
        select_local_directory_button = tk.Button(
            self.root, text="选择文件夹", command=self.select_local_directory
        )
        ...
        select_ssh_connection_button = tk.Button(
            self.root, text="选择连接", command=self.select_ssh_connection
        )
...
class SftpConnect:
    ...
    def init_sftp(self, host, pem=None, password=None):
        ...
        if pem:
            transport.connect(
                username=username, pkey=paramiko.RSAKey.from_private_key_file(pem)
            )
        else:
...
class Upload(SftpConnect):
    ...
    def scp_upload_file(self, local_path, remote_path, sftp):
        ...
        try:
            ...
            sftp.put(
                local_path,
                remote_path,
                callback=lambda transferred, total: self.upload_progress(
                    transferred, total, first_call
                ),
            )
    ...
    def upload_progress(self, transferred, total, first_call):
        ...
        if not first_call[0]:
            self.main.message_class.put(
                f"Progress: {transferred}/{total} bytes ({percentage:.2f}%)", None, True
            )
            return
        first_call[0] = False
        self.main.message_class.put(
            f"Progress: {transferred}/{total} bytes ({percentage:.2f}%)"
        )
        ...
        for root, dirs, files in os.walk(local_directory):
            for file in files:
                ...
                remote_path = os.path.normpath(
                    os.path.join(remote_directory, relative_path)
                )
            ...
            for directory in dirs:
                ...
                relative_subdirectory = os.path.relpath(
                    local_subdirectory, local_directory
                )
                remote_subdirectory = os.path.normpath(
                    os.path.join(remote_directory, relative_subdirectory)
                )
...
class DowdLoad(SftpConnect):
        ...
        try:
            ...
            if sftp.stat(remote_path).st_mode & 0o40000:
                local_path = os.path.join(
                    local_path, os.path.split(remote_path.rstrip("/"))[-1]
                )
            ...
            self.main.message_class.put(
                "File download completed successfully", Color.GREEN
            )
        finally:
            transport.close()
    ...
    def download_file(self):
        ...
        password = self.main.password_entry.get()
        threading.Thread(
            target=self.scp_download_thread,
            args=(remote_path, local_path, hostname),
            kwargs={"pem": pem, "password": password},
        ).start()
    ...
    def scp_download_file(self, remote_path, local_path, sftp):
        ...
        sftp.get(
            remote_path,
            local_path,
            callback=lambda x, y: self.main.message_class.put(
                f"Downloaded {x}/{y} bytes"
            ),
        )
    ...
    def download_files_recursively(self, remote_directory, local_directory, sftp):
        ...
        for file in files:
            ...
            if sftp.stat(remote_path).st_mode & 0o40000:
                ...
                self.main.message_class.put(
                    f"Downloading {remote_path} to {local_file_path}..."
                )
